...
export default function EditAssetScreen({ route, navigation }: any) {
  ...
  const loadData = async () => {
    try {
      const [assetResponse, deptResponse] = await Promise.all([
      ...
      setDepartmentId(asset.department_id.toString());
      setLocation(asset.location || '');
      setPurchaseDate(asset.purchase_date || '');
      setPurchasePrice(asset.purchase_price?.toString() || '');
      setDescription(asset.description || '');
      setStatus(asset.status);
      setExistingImageUrl(asset.image_url || '');
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await assetAPI.update(assetId, formData);
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.header}>
        <Title style={styles.title}>Edit Asset</Title>
      </View>
      ...
      <View style={styles.form}>
        ...
        <View style={styles.imageSection}>
          {image ? (
            ...
            <Image source={{ uri: getFullImageUrl(existingImageUrl) || '' }} style={styles.image} />
          ) : (
